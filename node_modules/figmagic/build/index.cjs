...
function loadFile(path7) {
  ...
  if (!import_fs.default.existsSync(path7)) throw Error(ErrorLoadFile(path7));
  const data = import_fs.default.readFileSync(path7, "utf8");
...
var sliceOutObjectFromFile = (path7) => {
  ...
  const data = import_fs2.default.readFileSync(path7, "utf8");
...
`;
    let imports = [];
    const ID = id || (0, import_crypto.randomUUID)().slice(0, 8);
...
var checkIfExists = (path7) => import_fs3.default.existsSync(path7);
...
function getSvgFileData(filePath) {
  ...
  if (!import_fs4.default.existsSync(`${process.cwd()}/${filePath}`)) return "";
  return import_fs4.default.readFileSync(`${process.cwd()}/${filePath}`, "utf8");
}
...
function createMissingFoldersFromPath(filePath) {
  ...
  if (!import_fs6.default.existsSync(directoryPath))
    import_fs6.default.mkdirSync(directoryPath, { recursive: true });
}
...
function write(filePath, fileContent) {
  ...
  import_fs7.default.writeFileSync(filePath, fileContent, "utf-8");
}
...
function refresh(folderPath, trashExistingFolder = true) {
  ...
  const resolvedPath = import_path.default.resolve(process.cwd(), folderPath);
  if (trashExistingFolder && import_fs8.default.existsSync(resolvedPath)) {
    if (process.versions.node >= "14.14.0")
      import_fs8.default.rmSync(resolvedPath, { recursive: true });
    else import_fs8.default.rmdirSync(resolvedPath, { recursive: true });
  }
...
async function request(urlPath, figmaToken) {
  ...
  return new Promise((resolve, reject) => {
    import_https.default.get(options, (resp) => {
      resp.on("data", (chunk) => {
...
function getFile(filePath) {
  return import_fs9.default.readFileSync(filePath);
}
...
async function downloadFile(url, filePath) {
  ...
  return new Promise((resolve, reject) => {
    const req = import_https2.default.get(url, (resp) => {
      if (resp.statusCode >= 200 && resp.statusCode < 300) {
        createMissingFoldersFromPath(filePath);
        const write2 = resp.pipe(import_fs10.default.createWriteStream(filePath));
...
function loadEnv() {
  ...
  if (import_fs11.default.existsSync(filePath)) {
    const file = import_fs11.default.readFileSync(filePath, "utf-8");
